...
export 'package:path_provider_ffi/src/provider.dart'
    show StorageDirectory, PathProvider;
...
PathProvider get _platform => PathProvider.instance;
...
Directory getTemporaryDirectory() {
  final String? path = _platform.getTemporaryPath();
  if (path == null) {
...
Directory getApplicationSupportDirectory() {
  final String? path = _platform.getApplicationSupportPath();
  if (path == null) {
    throw MissingPlatformDirectoryException(
...
Directory getLibraryDirectory() {
  final String? path = _platform.getLibraryPath();
  if (path == null) {
...
Directory getApplicationDocumentsDirectory() {
  final String? path = _platform.getApplicationDocumentsPath();
  if (path == null) {
    throw MissingPlatformDirectoryException(
...
Directory getApplicationCacheDirectory() {
  final String? path = _platform.getApplicationCachePath();
  if (path == null) {
    throw MissingPlatformDirectoryException(
...
Directory? getExternalStorageDirectory() {
  final String? path = _platform.getExternalStoragePath();
  if (path == null) {
    return null;
  }
  return Directory(path);
...
/// Android.
List<Directory>? getExternalCacheDirectories() {
  final List<String>? paths = _platform.getExternalCachePaths();
  if (paths == null) {
    return null;
  }
...
/// platform. This is unlikely to ever be supported on any platform other than
/// Android.
List<Directory>? getExternalStorageDirectories({
  /// Optional parameter. See [StorageDirectory] for more informations on
  /// how this type translates to Android storage directories.
  StorageDirectory? type,
}) {
  final List<String>? paths = _platform.getExternalStoragePaths(type: type);
  if (paths == null) {
    return null;
  }
...
Directory? getDownloadsDirectory() {
  final String? path = _platform.getDownloadsPath();
  if (path == null) {
    return null;
  }
  return Directory(path);
